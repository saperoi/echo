...
@tag.child
@lightbulb.add_checks(lightbulb.has_guild_permissions("MANAGE_GUILD") | lightbulb.owner_only)
@lightbulb.add_checks(lightbulb.guild_only)
...
async def rec(ctx: lightbulb.Context):
    ...
    if table_exist_check(str(ctx.guild_id)):
        curtag.execute("SELECT content FROM tag_" + str(ctx.guild_id) + " WHERE name=?", (re.sub(r'[^a-zA-Z0-9]', '',ctx.options.name),))
        r, = curtag.fetchone()
...
async def crt(ctx: lightbulb.Context):
    ...
    if table_exist_check(str(ctx.guild_id)):
        try:
            curtag.execute("INSERT INTO tag_" + str(ctx.guild_id) + " VALUES (?, ?, ?)", (re.sub(r'[^a-zA-Z0-9]', '',ctx.options.name), ctx.author.id, re.sub(r'[^a-zA-Z0-9,._!?():/ ]', '',ctx.options.content)))
...
async def dlt(ctx: lightbulb.Context):
    comm.log_com(ctx)
    if table_exist_check(str(ctx.guild_id)):
        curtag.execute("SELECT id FROM tag_" + str(ctx.guild_id) + " WHERE name=?", (re.sub(r'[^a-zA-Z0-9]', '',ctx.options.name),))
        i, = curtag.fetchone()
        if int(i) == int(ctx.author.id) or int(ctx.author.id) == 738772518441320460:
            curtag.execute("DELETE from tag_" + str(ctx.guild_id) + " where name=?", (re.sub(r'[^a-zA-Z0-9]', '',ctx.options.name),))
